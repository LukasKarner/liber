...
class TestAddCmd:
    def _add(self, runner, lib_dir, dummy_pdf, dummy_bib, extra=None):
        args = _base_args(lib_dir) + ["add", str(dummy_pdf), str(dummy_bib)]
        if extra:
            args += extra
        return runner.invoke(cli, args)

    def test_add_exits_zero(self, runner, lib_dir, dummy_pdf, dummy_bib):
        result = self._add(runner, lib_dir, dummy_pdf, dummy_bib)
    ...
    def test_add_with_custom_key(self, runner, lib_dir, dummy_pdf, dummy_bib):
        result = self._add(runner, lib_dir, dummy_pdf, dummy_bib, extra=["--key", "mycustomkey"])
        assert result.exit_code == 0
    ...
    def test_add_duplicate_fails(self, runner, lib_dir, dummy_pdf, dummy_bib):
        ...
        result = runner.invoke(cli, _base_args(lib_dir) + ["add", str(dummy_pdf2), str(dummy_bib)])
        assert result.exit_code != 0
    ...
    def test_add_no_doi_graceful(self, runner, lib_dir, tmp_path, dummy_pdf):
        ...
        result = runner.invoke(cli, _base_args(lib_dir) + ["add", str(dummy_pdf), str(bib)])
        assert result.exit_code == 0

    def test_add_missing_pdf_fails(self, runner, lib_dir, dummy_bib):
        result = runner.invoke(
            cli, _base_args(lib_dir) + ["add", "/nonexistent/file.pdf", str(dummy_bib)]
        )
        assert result.exit_code != 0

    def test_add_missing_bib_fails(self, runner, lib_dir, dummy_pdf):
        result = runner.invoke(
            cli, _base_args(lib_dir) + ["add", str(dummy_pdf), "/nonexistent/paper.bib"]
        )
        assert result.exit_code != 0
...
class TestListCmd:
    ...
    def test_list_shows_added_paper(self, runner, lib_dir, tmp_path, dummy_pdf):
        ...
        runner.invoke(cli, _base_args(lib_dir) + ["add", str(dummy_pdf), str(bib)])
        result = runner.invoke(cli, _base_args(lib_dir) + ["list"])
        assert result.exit_code == 0
